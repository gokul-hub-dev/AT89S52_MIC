...
//FRAME - A5 5A 00 00 FF AA 00
/*
FF - CHECKSUM(0,1,2,3,6 - BYTE)
AA - RX_Data[5] - END BYTE
00 - RX_Data[6] - FMU STATUS
*/
void Received_Data() {
  if (RX_Data[0] == IDENTITY_BYTE) {
    if (RX_Data[1] == START_BYTE) {
      unsigned char temp = RX_CHECKSUM(RX_Data[2], RX_Data[3],RX_Data[6]);
