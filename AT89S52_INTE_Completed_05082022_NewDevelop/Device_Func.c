...
void Rx_345Bit(UINT8 Rx_data3, UINT8 Rx_data4) //Checking 3,4,5 bits and greater or less then 3.
{
  //	UINT8 _port3;
  ...
  if (RX_Data[0] == IDENTITY_BYTE) {
    if (RX_Data[1] == START_BYTE) {
      ...
      if ((RX_Data[5] == temp) && (RX_Data[6] == END_BYTE)) {
        Rx_012Bit(RX_Data[2]); // receive "0xFF" convert to "0x00"(leds ON)
        Rx_345Bit(RX_Data[2], RX_Data[4]); // receive "0x00" convert to "0xFF"(leds OFF)
      }// else {
        //RX_Index=0;
